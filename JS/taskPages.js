...
function assignIndex() {
  ...
  if (
    ...
    selectElement.value === "/Images/KarenBjølstadInTheRockingChair-crop.jpg" ||
...
function generateDropdownItems() {
  ...
  for (var i = 0; i < currentFloorArray.length; i++) {
    ...
    link.setAttribute("tabindex", "0"); // Allow the link to receive focus
...
function navigateDropdown(e) {
  ...
  switch (
    e.key // Check which key was pressed
  ) {
    case "ArrowUp":
      e.preventDefault();
      if (currentIndex > 0) {
        // Check if the current index is greater than 0
        dropdownItems[currentIndex - 1].focus(); // Focus the previous item
        lastIndex = currentIndex - 1; // Set the last index to the current index
    ...
    case "ArrowDown":
      ...
      if (currentIndex < dropdownItems.length - 1) {
        // Check if the current index is less than the number of items
        dropdownItems[currentIndex + 1].focus(); // Focus the next item
        lastIndex = currentIndex + 1; // Set the last index to the current index
    ...
    case "Enter": // Enter key
      ...
      if (lastIndex !== -1) {
        // Check if an item has been selected using the arrow keys
        selectItem(dropdownItems[lastIndex].textContent); // Select the item
...
searchInput.addEventListener("focus", toggleDropdown(), filterItems());
searchInput.addEventListener("blur", toggleDropdown(), filterItems());
